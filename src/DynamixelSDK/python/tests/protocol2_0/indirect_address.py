...
if MY_DXL == "X_SERIES" or MY_DXL == "MX_SERIES":
    ...
    LEN_INDIRECTDATA_FOR_WRITE = (
        5  # Sum of Data of Length. i.e) LED (1 byte) + Goal Position data (4 bytes)
    )
...
groupSyncWrite = GroupSyncWrite(
    portHandler, packetHandler, ADDR_INDIRECTDATA_FOR_WRITE, LEN_INDIRECTDATA_FOR_WRITE
)

# Initialize GroupSyncRead instace for Present Position
groupSyncRead = GroupSyncRead(
    portHandler, packetHandler, ADDR_INDIRECTDATA_FOR_READ, LEN_INDIRECTDATA_FOR_READ
)
...
dxl_comm_result, dxl_error = packetHandler.write2ByteTxRx(
    portHandler, DXL_ID, ADDR_INDIRECTADDRESS_FOR_WRITE + 0, ADDR_GOAL_POSITION + 0
)
if dxl_comm_result != COMM_SUCCESS:
    print("%s" % packetHandler.getTxRxResult(dxl_comm_result))
elif dxl_error != 0:
    print("%s" % packetHandler.getRxPacketError(dxl_error))
dxl_comm_result, dxl_error = packetHandler.write2ByteTxRx(
    portHandler, DXL_ID, ADDR_INDIRECTADDRESS_FOR_WRITE + 2, ADDR_GOAL_POSITION + 1
)
if dxl_comm_result != COMM_SUCCESS:
    print("%s" % packetHandler.getTxRxResult(dxl_comm_result))
elif dxl_error != 0:
    print("%s" % packetHandler.getRxPacketError(dxl_error))
dxl_comm_result, dxl_error = packetHandler.write2ByteTxRx(
    portHandler, DXL_ID, ADDR_INDIRECTADDRESS_FOR_WRITE + 4, ADDR_GOAL_POSITION + 2
)
if dxl_comm_result != COMM_SUCCESS:
    print("%s" % packetHandler.getTxRxResult(dxl_comm_result))
elif dxl_error != 0:
    print("%s" % packetHandler.getRxPacketError(dxl_error))
dxl_comm_result, dxl_error = packetHandler.write2ByteTxRx(
    portHandler, DXL_ID, ADDR_INDIRECTADDRESS_FOR_WRITE + 6, ADDR_GOAL_POSITION + 3
)
if dxl_comm_result != COMM_SUCCESS:
    print("%s" % packetHandler.getTxRxResult(dxl_comm_result))
elif dxl_error != 0:
    print("%s" % packetHandler.getRxPacketError(dxl_error))
dxl_comm_result, dxl_error = packetHandler.write2ByteTxRx(
    portHandler, DXL_ID, ADDR_INDIRECTADDRESS_FOR_WRITE + 8, ADDR_LED_RED
)
if dxl_comm_result != COMM_SUCCESS:
    print("%s" % packetHandler.getTxRxResult(dxl_comm_result))
elif dxl_error != 0:
...
dxl_comm_result, dxl_error = packetHandler.write2ByteTxRx(
    portHandler, DXL_ID, ADDR_INDIRECTADDRESS_FOR_READ + 0, ADDR_PRESENT_POSITION + 0
)
if dxl_comm_result != COMM_SUCCESS:
    print("%s" % packetHandler.getTxRxResult(dxl_comm_result))
elif dxl_error != 0:
    print("%s" % packetHandler.getRxPacketError(dxl_error))
dxl_comm_result, dxl_error = packetHandler.write2ByteTxRx(
    portHandler, DXL_ID, ADDR_INDIRECTADDRESS_FOR_READ + 2, ADDR_PRESENT_POSITION + 1
)
if dxl_comm_result != COMM_SUCCESS:
    print("%s" % packetHandler.getTxRxResult(dxl_comm_result))
elif dxl_error != 0:
    print("%s" % packetHandler.getRxPacketError(dxl_error))
dxl_comm_result, dxl_error = packetHandler.write2ByteTxRx(
    portHandler, DXL_ID, ADDR_INDIRECTADDRESS_FOR_READ + 4, ADDR_PRESENT_POSITION + 2
)
if dxl_comm_result != COMM_SUCCESS:
    print("%s" % packetHandler.getTxRxResult(dxl_comm_result))
elif dxl_error != 0:
    print("%s" % packetHandler.getRxPacketError(dxl_error))
dxl_comm_result, dxl_error = packetHandler.write2ByteTxRx(
    portHandler, DXL_ID, ADDR_INDIRECTADDRESS_FOR_READ + 6, ADDR_PRESENT_POSITION + 3
)
if dxl_comm_result != COMM_SUCCESS:
    print("%s" % packetHandler.getTxRxResult(dxl_comm_result))
elif dxl_error != 0:
    print("%s" % packetHandler.getRxPacketError(dxl_error))
dxl_comm_result, dxl_error = packetHandler.write2ByteTxRx(
    portHandler, DXL_ID, ADDR_INDIRECTADDRESS_FOR_READ + 8, ADDR_MOVING
)
if dxl_comm_result != COMM_SUCCESS:
    print("%s" % packetHandler.getTxRxResult(dxl_comm_result))
elif dxl_error != 0:
...
while 1:
    ...
    while 1:
        ...
        dxl_getdata_result = groupSyncRead.isAvailable(
            DXL_ID, ADDR_INDIRECTDATA_FOR_READ, LEN_PRESENT_POSITION
        )
        ...
        dxl_getdata_result = groupSyncRead.isAvailable(
            DXL_ID, ADDR_INDIRECTDATA_FOR_READ + LEN_PRESENT_POSITION, LEN_MOVING
        )
        if dxl_getdata_result != True:
            print("[ID:%03d] groupSyncRead getdata failed" % DXL_ID)
            quit()
        ...
        dxl_moving = groupSyncRead.getData(
            DXL_ID, ADDR_INDIRECTDATA_FOR_READ + LEN_PRESENT_POSITION, LEN_MOVING
        )

        print(
            "[ID:%03d] GoalPos:%d  PresPos:%d  IsMoving:%d"
            % (DXL_ID, dxl_goal_position[index], dxl_present_position, dxl_moving)
        )

        if not (
            abs(dxl_goal_position[index] - dxl_present_position)
